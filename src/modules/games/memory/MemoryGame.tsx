...
import { Box, Button, FormControl, InputLabel, MenuItem, Select, SelectChangeEvent, Stack, Typography, IconButton } from '@mui/material';
...
const MemoryGame: React.FC<MemoryGameProps> = ({ onClose }) => {
  ...
  const startNewGame = (overridePairs?: number) => {
    ...
    const count = Math.max(2, Math.min(overridePairs ?? pairsCount, TOTAL_PAIRS));
  ...
  return (
    <Box sx={{ width: '100%', height: '100vh', display: 'flex', flexDirection: 'column', p: { xs: 1, sm: 2 } }}>
      ...
      <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
        <Typography variant="h6">JJJ Memory</Typography>
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <FormControl size="small" sx={{ minWidth: 120 }}>
            ...
            >
              <MenuItem value={8}>8 (16 cards)</MenuItem>
              <MenuItem value={12}>12 (24 cards)</MenuItem>
              <MenuItem value={16}>16 (32 cards)</MenuItem>
            </Select>
          </FormControl>
          <Button variant="outlined" onClick={() => startNewGame()}>New Game</Button>
        </Stack>
      </Stack>

      {/* Score */}
      <Stack direction="row" spacing={3} sx={{ mb: 2, px: 0.5 }}>
        <Typography variant="body2">Moves: {moves}</Typography>
        <Typography variant="body2">Solved: {matchedPairIds.size}/{deck.length / 2}</Typography>
        {allSolved && <Typography variant="body2" color="success.main">Completed!</Typography>}
      </Stack>

      {/* Grid */}
      <Box sx={{ flex: 1, display: 'grid', gap: 1.2, gridTemplateColumns: {
        xs: 'repeat(4, 1fr)',
        sm: 'repeat(6, 1fr)',
        md: 'repeat(6, 1fr)',
        lg: 'repeat(8, 1fr)'
      ...
      {matchOverlay && (
        <Box sx={{
          ...
          <Box sx={{
            ...
            <Box sx={{ width: { xs: '42vw', sm: 260 }, maxWidth: 340, aspectRatio: '440 / 597', borderRadius: '16px', overflow: 'hidden', boxShadow: 8, animation: 'zoomIn 600ms cubic-bezier(0.2, 0.8, 0.2, 1)' }}>
